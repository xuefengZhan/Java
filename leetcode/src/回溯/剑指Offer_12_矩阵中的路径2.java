 ...
 * 7.2%
 */
public class 剑指Offer_12_矩阵中的路径2 {
    ...
    public boolean exist(char[][] board, String word) {
        ...
        for (int i = 0; i < board.length; i++) {
            for (int j = 0; j < board[i].length; j++) {
                ...
                visited[i][j] = 1;
                //todo 3.dfs
                place(word,board,i,j,visited,flag);
                if(flag[0] == word.length()) return true;
            }
        }
        return false;
    }
    ...
    public void place(String word,char[][] board,int row,int col, byte[][] visited,int[] flag){
        ...
        for(int i = -1;i<=1;i++){
            for(int j = -1;j<=1;j++){
                ...
                if(board[row + i][col + j] != word.charAt(flag[0])) continue; // 排除不相等的

                visited[row + i][col + j] = 1;

                // todo res.size() 是核心！  res一旦有元素，所有回溯递归过程都直接return  直接跳出递归调用栈 回到栈顶！
                flag[0] = flag[0] + 1;
                place(word,board,row+i,col+j,visited,flag);
    ...
    public static void main(String[] args) {
        ...
        剑指Offer_12_矩阵中的路径2 v = new 剑指Offer_12_矩阵中的路径2();
